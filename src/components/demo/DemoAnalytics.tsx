...
import { Download, BarChart3, Users, MessageSquare, TrendingUp, AlertTriangle, FileText, ArrowLeft } from "lucide-react";
...
export const DemoAnalytics = ({ onBackToMenu }: DemoAnalyticsProps) => {
  ...
  const handlePDFExport = async () => {
    ...
    const surveyName = surveys?.find(s => s.id === filters.surveyId)?.title || "Demo Survey";
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-muted-foreground mt-1">Real-time insights from demo conversations</p>
            </div>
            ...
            <TabsContent value="overview" className="space-y-6">
              {isLoading ? (
                ...
                <EmptyState
                  ...
                  description="Start a demo conversation to see analytics here."
                  actionLabel="Try Employee Demo"
                  onAction={() => navigate('/demo/employee')}
                />
              ) : participation?.totalAssigned === 0 ? (
                <EmptyState
                  icon={Users}
                  title="No Demo Responses Yet"
                  description="Complete a demo conversation to see analytics here."
                  actionLabel="Try Employee Demo"
                  onAction={() => navigate('/demo/employee')}
                />
              ) : (
            ...
            <TabsContent value="urgency">
              {!urgency || urgency.length === 0 ? (
                <EmptyState
                  ...
                  description="Great news! There are no urgent issues requiring immediate attention in the demo data."
                />
              ) : (
